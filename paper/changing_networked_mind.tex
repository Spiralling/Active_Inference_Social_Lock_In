...
% ======================================================================
\section{Models of collective belief revision}
\label{sec:background}
% ======================================================================

The question of how a community evaluates, adopts, and discards a system of knowledge has, over the
last two decades, migrated from the philosophy of science into computational social science, where
the qualitative insights of Kuhn, Lakatos, and Feyerabend have been recast as agent-based and
network models one can actually run. It is worth orienting against that body of work before adding
to it, because the contribution we intend is sharply delimited by what these models already do well
and by two things they do not do at all.

The dominant formalism is \emph{network epistemology}: a community of agents, each holding a scalar
credence over a small menu of competing hypotheses, repeatedly tests, updates by Bayes, and shares
results with network neighbours \citep{zollman2010,oconnor2019}. The celebrated results of this
program concern the surprising epistemic value of inefficiency. Zollman's effect---that sparsely
connected communities often outperform densely connected ones, because slow information flow shields
pockets of the network from early misleading data and so preserves transient diversity---vindicates,
in mathematics, the old intuition that a field benefits when some of its members stubbornly resist
the consensus. A parallel literature on the \emph{division of cognitive labour} shows that a
community hedges its bets best not when its scientists reason as dutiful truth-seekers but when they
chase reward, scattering across approaches that pure epistemic rationality would have left unstaffed
\citep{kitcher1990,strevens2003}. And a more recent strand identifies a whole taxonomy of
mechanisms---sparse topology, large populations, strong priors, adaptive learning rules, intermittent
ties---that stave off premature convergence \citep{jonard2025}. Alongside these run the
active-inference treatments of collective cognition, in which \emph{epistemic communities} develop
shared belief through coupled free-energy minimization \citep{albarracin2022}, and the recent
formalization of belief revision itself as carrying a cost \citep{hyland2025}. Decentralized-science
efforts, finally, take these coordination questions out of simulation and into the design of real
research institutions.

Two features cut across nearly all of this work, and they are exactly the two we wish to change.
The first is that \emph{exploration is posited rather than derived}. To keep a community from
collapsing onto the first locally attractive theory, these models equip their agents with an
exploration term---the $\epsilon$ of an $\epsilon$-greedy rule, a temperature, a switching cost, an
exogenous prior strength. The diversity that protects the community is bought with a free parameter.
The second, and deeper, feature is that \emph{the hypothesis space is fixed}. The agents update over
a menu---theory A versus theory B, a handful of arms on a bandit---that the modeller wrote down in
advance. The interesting dynamics live in how the topology of communication interacts with movement
\emph{within} that menu. What no one studies is a community whose menu is \emph{itself} the thing
being learned, revised, and transmitted. The room is nearly empty, and the reason it is empty is
that there has been no cheap, principled mechanism for ``learning what the model even is'' that could
be run across a population.

% ======================================================================
\section{Hidden structure and the necessity of structure learning}
\label{sec:hidden}
% ======================================================================

Return to Kuhn's deepest observation, which the scalar models gesture at but cannot embody: a
paradigm shapes what a scientist can perceive, what she chooses to investigate, and which
alternatives she will entertain---three faces, he insists, of a single fact \citep{kuhn1962}. The
reason a scalar credence over a fixed menu cannot carry this is that a menu has no hidden interior.
It can represent which theory is favoured; it cannot represent the dependencies that make one theory
\emph{cost} more to abandon than another, nor the way a commitment at the centre of a web of
inference quietly determines what counts, at the periphery, as an anomaly worth noticing.

But there is a prior problem, and it is the one we want to make the engine of the paper, because it
makes structure learning not a stylistic preference but a necessity. \emph{The structure of reality
is hidden.} A scientist confronting a new domain is not given the right variables, and she is
certainly not given the dependencies among them. She does not know, in advance, whether two observed
regularities share a hidden common cause, are directly and lawfully coupled, or merely coincide. The
generative model whose structure she must eventually come to hold is, at the outset, unknown in its
very shape. This poses a question the fixed-menu models never have to face: if you do not know the
structure of the world, how do you build a network over it that you can sample from at all?

The answer forces everything that follows. You begin with \emph{some} network---a provisional Bayes
net of commitments---and you treat its structure as itself an object of inference, to be expanded
when the data demand a commitment you had not represented and reduced when a dependency you had
posited earns no evidence. Learning a model from scratch, when the structure is hidden, is
\emph{Bayesian model expansion and reduction}. And this immediately discharges the first of our two
debts to the prior literature: the exploration term that those models install by hand reappears here
with a mechanism behind it. To explore is to \emph{expand}---to entertain a node or an edge not yet
in the model---and the drive to do so is the epistemic value of the expansion, the expected
information it would yield, which active inference already supplies through expected free energy
\citep{friston2015,millidge2021}. We do not add an exploration bonus; we identify exploration with
the structural move of proposing a new piece of the world.

\paragraph{Desiderata.}
The construction that follows is built to satisfy six requirements:
\begin{enumerate}
\item that a paradigm be a \emph{structured} object---a network of interdependent commitments with a
dense core and a sparse belt in contact with observation;
\item that revising a commitment incur a \emph{carry-over} cost---the aggregate of the re-readings it
forces on everything downstream;
\item that \emph{conservatism be derived} from structural position rather than assigned by a dial;
\item that \emph{structure learning be principled}---Bayesian model reduction over a continuous
precision space whose corners are the discrete topologies;
\item that \emph{conviction be a second field on the same network}, decoupled from conservatism and
written into the model by the same expansion that writes a new belief;
\item that the \emph{population signature} be a graded reorganization, or a stall, rather than a
single thresholded flip.
\end{enumerate}
The active-inference machinery enters unmodified throughout. Section~\ref{sec:results} reports how
each desideratum fares once the model is actually run.
