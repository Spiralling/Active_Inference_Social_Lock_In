...
discovery---in a 2{,}056-run sweep yields three findings. The lock-in boundary sits at \emph{very}
...
follows a Poisson waiting-time law, but a \emph{staggered} discovery never survives the communal
prior---a concept persists only when the community wakes it near-simultaneously, a critical mass in
\emph{time}. And one clean negative: a conservatism-gated revision rate does \emph{not} recover the
predicted belt-first/core-last staircase, which localizes the missing ingredient to data-dependent
structure learning.
\end{abstract}
...
Section~\ref{sec:results} reports a single-agent dark-energy net and a 2{,}056-run population sweep
...
operation---propagating influence from a commitment to everything downstream---generates \emph{two}
fields on the same graph, depending on what is propagated. Propagate sheer dependence and we get
...
$\pi_{uv}$ measuring how tightly $v$ is bound to $u$. Internal coherence is a precision-weighted
graphical model, $p(x\mid G)\propto\prod_{(u\to v)\in E}\psi_{uv}(x_u,x_v)^{\pi_{uv}}$. The dense,
high-precision interior is the \emph{core}; the sparse periphery in contact with observation is the
\emph{belt}, each belt node reading the world through a sensory channel of precision $\rho_k$. The
core carries no direct sensory channel: an interior commitment is revised only by error propagated
inward from the belt, which is what makes it slow. We work throughout in a \emph{linear-Gaussian}
regime, which puts every quantity below in closed form (Appendix~\ref{app:operator}); agents are
\emph{honest}---no one misreports, so lock-in is never a dishonesty artifact---and \emph{bounded},
minimizing variational free energy step by step \citep{dacosta2020} rather than planning over
policies.

Revising a commitment is not a local act: changing $x_v$ forces everything that depends on it to
re-equilibrate. The whole cascade is generated by one object. From the adjacency $A$, the directed
\emph{propagation operator} is $\Top=(I-A)^{-1}$, whose entry $\Top_{iw}=\pi_{i\rw w}$ is the total
precision-weighted coupling propagated from $i$ to a descendant $w$ along all directed paths
(Appendix~\ref{app:operator}). Conservatism is $\Top$ applied to the unit source,
\begin{equation}
\kappa \;=\; \Top\,\bone,
\qquad
...
\label{eq:kappa}
\end{equation}
the precision-weighted mass of a node's descendants: a core node is costly to revise because much
depends on it, a belt node cheap because little does. Conservatism is thus \emph{derived} from
connectivity, not assigned by hand.

The second field is value. Scientists are bounded agents who value some conclusions over others---a
result on which a career rests is held more tightly than its evidence warrants
\citep{kitcher1990,strevens2003}. Following the variational-cost account of belief revision
\citep{hyland2025}, we attach to each node an intrinsic utility $u_i$ and let it propagate through the
\emph{same} operator: a commitment one wants true lends value to its dependents, so the conviction
field is
\begin{equation}
U \;=\; \Top\,u,
\qquad
...
\label{eq:U}
\end{equation}
$\Top\bone$ and $\Top u$ are linearly independent unless $u\propto\bone$: the two fields need not
align (Fig.~\ref{fig:twofields}), and Appendix~\ref{app:darkenergy} exhibits the misalignment
computed on a real net.
...
\label{eq:tilted}
\end{equation}
with $\lambda$ a single temperature governing how far value may bend the evidence. In the
linear-Gaussian regime the tilt is a pure mean shift, displacing the commitments by $\lambda\Sigma U$
and leaving the covariance untouched (Appendix~\ref{app:operator}). What opposes the displacement is
...
to expand: candidates arrive at a Poisson rate and are scored by $\Delta F$. It must be a rate,
...
also-saturated per-step evidence, so conviction holds only above a threshold: in earlier population
runs the entrenched bloc re-tracked the truth for $\lambda\le6$ and stayed locked for
$\lambda\ge15$---motivated persistence turned from a posited bias into a threshold one must clear.
...
evidence and its neighbours' carry weight in proportion to trust---and structural edits spread the
...
\item \textbf{Crisis check (reduction).} The closed-form prune evidence $\Delta F$
(Eq.~\ref{eq:bmr}) on the hub's edges is compared against the agent's conviction protection
$\lambda_i v_e$; a crossing is a \emph{crisis}, and the agent applies the reduction---the hub is
released.
\item \textbf{Discovery check (expansion).} After reduction, the residual the old paradigm absorbed
becomes visible as a coherent floor in prediction error; when the floor clears the expansion trigger
(and a Poisson proposal has arrived, rate $r$), the agent wakes the unconceived node and wires it in
(\S\ref{sec:structurelearning}).
...
model's own evidence. Table~\ref{tab:dials} lists the dials swept; everything else is held fixed, and
...
Expansion trigger & --- & residual floor required to wake a new node \\
...
\hline
\end{tabular}
\caption{The dials swept in the experiments (2{,}056 runs); all other quantities are held fixed.}
\label{tab:dials}
\end{table}
...
phlogiston world, swept overnight over the dials of Table~\ref{tab:dials} (2{,}056 healthy runs).
...
limitations and the model's one clean negative (\S\ref{sec:res-staircase}).
...
arc lives in the dial space, and four findings organize it.
...
weaker than trust within the community---more than $93\%$ convert, no matter how strong the gate,
and making the agents twice as dogmatic ($\lambda_d:0.35\to0.8$) barely shifts the picture
...
$5\%$ of the population lifts end-state conversion from $0.60$ to $0.79$, because the vanguard's
...
\paragraph{Discovering the unconceived is cheap; keeping it requires discovering together.}
...
rate $r$, \S\ref{sec:structurelearning}). Individually this behaves exactly as a waiting process
should: the fraction of the open community that discovers within the horizon follows the textbook
law $1-(1-r)^T$, rising from $12\%$ at $r=0.01$ to $88\%$ at $r=0.16$
(Fig.~\ref{fig:res-waiting}, left); and because $\sim$40 agents draw in parallel, \emph{somebody}
wakes the node within a few steps at every rate (right panel)---the community's first discovery is
nearly free. What is not free is survival. A discovery made by one agent at a time \emph{never}
survives, at any rate (the red curve): the moment the lone discoverer fuses beliefs with peers who
lack the concept, its oxygen node is averaged away---the shared prior treats the slot the others do
not represent as a firmly held ``no such thing,'' and pulls the awakened coupling back to zero. Only
when the proposal wakes the community near-simultaneously does the concept take root ($47\%$ of runs
under simultaneous proposal). A new concept does not need a critical mass of believers eventually;
it needs them at the same \emph{time}. (One modeling caveat: under posterior fusion the unconceived
slot's pin is pooled as if it were a held belief. This reads naturally as incommensurability---the
shared conceptual scheme suppresses concepts it does not contain---but fusion schemes that exclude
unconceived dimensions would soften the effect.)
...
the open community that discovers oxygen follows the waiting-time law $1-(1-r)^T$ (dashed), but the
fraction whose concept \emph{survives} fusion is zero at every Poisson rate (red): staggered
discoverers are pulled back by the communal prior, while simultaneous proposal (point, right edge)
survives in $47\%$ of runs---critical mass in \emph{time}. \emph{Right:} with $\sim$40 agents drawing
in parallel, the population's first discovery is nearly free at any rate.}
\label{fig:res-waiting}
\end{figure}

\paragraph{The cycle is robust; the one fragile dial is how loudly the world must knock.}
Re-running the full arc across forgetting and observation noise ($\omega\in[0.9,1.0]$,
$\sigma_o\in[0.25,1.0]$) changes nothing: crisis, reduction and discovery all still arrive, even in
the no-forgetting corner $\omega=1$. The one dial that can break the cycle is the expansion
trigger---how large an unexplained residual an agent demands before it will even entertain a new
node. Set at $1.0$--$1.5$ the cycle completes everywhere; at $2.5$ the bar sits \emph{above} the
prediction error the dead paradigm actually leaves behind after reduction, so the new node is never
proposed and discovery dies almost everywhere (fraction $\le0.42$, mostly $0$). The calibration
lesson is simple: an agent must be willing to consider new structure at the level of error its old
structure already produces.

\subsection{Limitations and future work}
\label{sec:res-staircase}

One prediction fails cleanly, and it names the next modelling target. The structural account
predicts a belt-first, core-last \emph{staircase}: the cheap periphery reorganizes first, the costly
core follows once enough evidence accumulates to pay its carry-over. A conservatism-gated revision
rate does \emph{not} recover it: the high-$\kappa$ nodes reorganize \emph{first}, and the
$\kappa$-gradient gate is indistinguishable from uniform and shuffled-$\kappa$ nulls
(Appendix~\ref{app:staircase} gives the full analysis). The root cause is that the Fisher deposit is
set by the observation operator rather than the data, so theory identity rides the posterior means
and $\kappa$, though real and computable (Appendix~\ref{app:darkenergy}), never enters the update.
The first item of future work is therefore a \emph{data-dependent} observation model in which the
couplings themselves are learned, making conservatism load-bearing in the dynamics. The second is
the utility source $u$, here exogenous and static: letting it accrete onto structurally entrenched
commitments---$u$ aligning with $\kappa$ as a programme matures---would recover Lakatos's
progressive and degenerating problemshifts and meet Laudan's reticulated co-variation of theory,
method, and aim \citep{lakatos1970,laudan1984}.

The model is, finally, stylized: the nets are illustrative, their weights chosen to expose the
geometry; the dials of Table~\ref{tab:dials} are swept, not inferred; and a community that
``tracks'' the eventually-true theory is committed to it, not thereby correct. What clears the
``more than a redescription'' bar is not any single result but the conjunction a scalar credence
cannot voice: the Schur-complement degeneracy (a scalar has no off-diagonal to fill), the
derived-not-imposed lock-in with its deaf-but-honest control, and the expansion that wakes an
unconceived node only when the world makes its absence felt.
...
precisely the absence of the common measure the network presupposes \citep{kuhn1983}. The staircase
null (\S\ref{sec:res-staircase}) is the tell: graded reorganization on fixed wiring is a Lakatosian
