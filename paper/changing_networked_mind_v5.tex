...
\begin{abstract}
How does a scientific community change its mind---not about a fact, but about the \emph{frame}
its facts live in? We model a paradigm as a directed network of commitments and a community as
bounded active-inference agents who learn that network's structure while valuing some
conclusions over others and pooling beliefs with peers. Two classical theorems make this
community a null regime: Gaussian beliefs under precision-weighted pooling can neither reject a
conflicting source nor sustain a school. Result 1 measures the consequence, the
\emph{homogeneity effect}: communities that would learn two crisply different wirings apart
collapse, under any contact, into one compromise net no member holds. Result 2 measures the
repair, in two stages: infer each source's \emph{reliability} from surprise and distinctness
returns as a timescale; \emph{represent the rival}---candidate wirings, a posterior over them,
hypotheses communicated rather than averaged---and it returns as a wall, two internally certain
camps at a genuine fixed point. Gates buy time; represented rivals buy walls. Averaging refines
a frame; it cannot grow a new one.
...
can do science at all, how a community comes to represent a genuinely new dimension of the
...
aggregation dynamics preserve open-endedness and which foreclose it. This paper builds that
foundation, and its central result is a negative one with constructive content: the most
natural baseline---Gaussian beliefs under precision-weighted pooling---provably lacks the
capacity, and even the cheapest repair the failure names---reliability inferred from
surprise---restores distinctness only as a timescale; a wall appears only when the rival itself
is represented, which we demonstrate at the smallest scale that exhibits it. Averaging refines
a frame; it cannot grow a new one.
...
named but could not run forward \citep{kuhn1962}: nothing in \emph{The Structure of Scientific
Revolutions} says, mechanistically, why a community holds a structure of commitments past the
first disconfirming evidence, nor what finally makes the holding give way. When the chemical
community gave up phlogiston it did not revise a single credence: it rewrote a network in which
combustion, calcination, respiration, and the theory of acids were all load-bearing on a hub
that had to go \citep{blumenthal2017,holmes2000,boantza2011}. Phlogiston was load-bearing for
too much.
...
scope (\S\ref{sec:structurelearning}), so novelty, where the experiments need it, is supplied
rather than conceived. They are \emph{bounded}, sampling a complex
...
field's own default choices; Section~\ref{sec:results} opens with the two theorems under which
those defaults can neither reject a conflicting source nor sustain a school---\emph{conflict
resolves into confident compromise}---and reports two results: the homogeneity effect, and its
two-stage repair, reliability inferred from surprise (a timescale) and represented rivals (a
wall). The discussion draws the map: what filling the representational slot bought, and what
remains.
...
``group'' as between-subject neuroimaging \citep{friston2016peb}; a \emph{community} of
structure learners is the open move this paper makes---deliberately restricted to the reduction
half, because the question prior to expansion is what reduction alone already forces: which
aggregation of Bayes nets lets distinct structures survive contact at all
...
What does one of those private networks hold? The
...
determined by the precision geometry rather than decreed. The core carries no direct sensory
...
rest---under the forgetting below, a \emph{saturating} one while evidence keeps arriving. A
bias, not a resistance; the results make that distinction load-bearing.

\label{sec:structurelearning}%
Structure learning revises $G$ itself---the first requirement of \S\ref{sec:hidden}. Two
...
\emph{Expansion}---entertaining a commitment no one has represented
\citep{friston2017curiosity}---is deliberately not modelled, and nothing below runs it. The
reason is scope, not principle: expansion is not closed-form (a new node may enlarge the
likelihood's support and requires a real inversion), the arrival of candidates raises
Stanford's problem of unconceived alternatives \citep{stanford2006}, and an honest treatment
is a paper of its own. The question here comes first in the pipeline: a community running
Bayesian model \emph{reduction} must aggregate its members' Bayes nets, and everything below
turns on whether that aggregation can keep two structures distinct at all. Where an experiment
needs novelty it is supplied by hand---the rival candidates of \S\ref{sec:rivals} are given,
not discovered. (The phlogiston prototype of Appendix~\ref{app:null} retains a Poisson-arrival
expansion check for the full Kuhnian cycle, derived in Appendix~\ref{app:crisis-discovery}; no
main-text result rests on it.)

\label{sec:temporal}%
Finally, \emph{forgetting}: accumulated precision relaxes toward the prior,
...
structure is moving \citep{mathys2011}. Without it $\kappa=\Top\bone$ can only grow and lock-in
...
the appendix's phlogiston prototype runs the full cycle with a Poisson-arrival expansion
check.
...
wiring, and the distance between them grows large and stays. Connected, pooling every round
(red), the divergence never forms: each fusion step averages the two half-formed wirings into
a compromise network neither community's evidence supports---a tenth of the disconnected
distance. This is Result 1, the \emph{homogeneity effect}: under default pooling a connected
population holds one network, whatever diversity its members were learning, because the
contraction theorem leaves the dynamics nowhere else to go.

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\linewidth]{fig_divergence_traces.png}
...
model. \emph{Conviction does not protect:} valuing a conclusion shifts where beliefs settle,
never when they yield \citep{hyland2025}. \emph{Isolation is the only protection, and it is
fragile:} at \emph{inter}$=0.005$---trust toward outsiders two hundred times weaker than
within---$92\%$ of a dogmatic community converts, because an agent can silence its own
instruments but not its neighbours'. \emph{Discovery does not survive company:} a concept
woken by a single agent is averaged away by peers who treat ``has no such variable'' as
``certainly zero.'' \emph{No added mechanism escapes:} adequacy-driven exploration, social
excitation, audited curiosity, and value accretion each re-time the passage to consensus or
synchronize the population for it. The effect even has a one-line algebraic face:
$|\,\mathrm{mean}_i\,\Pi_i\,|\le\max_i|\Pi_i|$ entrywise, so an average can never
\emph{create} an edge absent from every individual---whatever a collective knows beyond its
members lives in the \emph{union} of their networks, never in their average.
...
First, switch on the gate of \S\ref{sec:reliability}. Within an agent, one channel of a
three-commitment chain turns persistently contradictory at step $30$: the Gaussian baseline
does what Theorem 2 says it must---it settles confidently between the channel's report and its
own couplings, variance falling monotonically, the conflict leaving \emph{no trace}---while
the gated agent infers the channel unreliable ($\eta\to0.16$) and its uncertainty does what no
light-tailed posterior can: it \emph{rises} while evidence accumulates, peaking before the
belief reorganizes around the discounted channel (Fig.~\ref{fig:res-variance},
Appendix~\ref{app:reliability}). There is now a slot in the belief state for being
torn---crisis as suspended confidence, rendered as a posterior.

Between communities, point the same functional at the \emph{wiring}---the contested couplings
of $\Pi$---and the collapse of Fig.~\ref{fig:res-divergence} reverses (green): $94\%$ of the
disconnected divergence survives full contact where fixed trust keeps $10\%$. What the gate
\emph{reads} decides: pointed at \emph{opinions} it never fires here ($10.2\%$), because both
camps fit the same blended data and agree in their means---a pluralism that lives in wiring is
invisible to a trust that reads opinions.

But the gate is memoryless and its weights stay strictly positive, so the contraction theorem
still owns $t\to\infty$: every school it sustains is metastable. Figure~\ref{fig:res-walls}
makes the hierarchy visible on one long run---two camps in full contact, each running only its
own commitment's experiment (theory-laden evidence, the Kuhnian condition), three
architectures on byte-identical streams. Plain fusion collapses within a few steps: the
theorem. The trust gate holds the gap for hundreds of steps, then visibly decays: time, bought
and spent. The rival layer of \S\ref{sec:rivals} holds a flat gap to $t=1500$, each camp
internally certain of its own theory: a wall---a fixed point of the damped hypothesis-pooling
dynamics, not a slow leak (Appendix~\ref{app:reliability}).
...
then $N=16$, on hand-wired worlds---existence proofs that the wall was representational, not
sweeps of social topologies. No model expansion runs anywhere in the main text
...
never shifts produces no doubt; gates and rivals off reproduce the null regime
...
of tuning. Result 1 is the theorems made visible: a community built from its field's defaults
is the formalism's \emph{normal-science} limit, and nothing in it entrenches, diverges, or
breaks. Result 2 shows the wall stands exactly where the theorems put it: make reliability a
latent and an agent can be surprised into doubt, a community can hold two wirings under
contact---yet every protection the gate wins is a clock; represent the rival itself and a wall
finally stands.

\paragraph{Trust as inferred precision---now measured, and bounded.}
The gate is the conflict-resolution literature's canonical repair \citep{ohagan2012}: within
an agent it buys crisis rendered as a posterior; between agents, stable schools on a connected
graph holding shared data. But everything it buys, it buys as \emph{time}: the gate is
memoryless and its weights stay strictly positive, so consensus remains the only place the
dynamics can rest.

\paragraph{The missing slot is representational---and fillable.}
There is no coordinate in $(\Pi,h)$ whose value can mean \emph{torn between two hypotheses}:
conflict arrives as growing confidence, and fusion erases who disagreed about what. This
impoverishment is by representation, not parameters---why the gate could only slow it.
The rival layer of \S\ref{sec:rivals} is that diagnosis made executable, and the wall of
Fig.~\ref{fig:res-walls} is what filling the slot buys: multiple structural hypotheses per
agent (a genuinely torn agent), divergence between live candidates as a computable state, and
communication of hypotheses---fusion within frames, never averaging across them. What remains
open is named: a lone discoverer's candidate is majority-swamped under log-linear pooling, and no agent yet
\emph{chooses} the experiment on which its candidates disagree most. Gates buy time;
represented rivals buy walls---and on an aggregation that can hold branches, the model
expansion this paper deferred becomes worth building.
...
point, the gestalt switch given its slot (a $q(m)$ that flips on prequential evidence).

Any community whose aggregation step averages rather than gates inherits the regime's
signature, whatever the sophistication of its members: a population of structure learners is
only as \emph{open-ended} as its pooling permits---and as what its pooling \emph{reads}
(\S\ref{sec:res-distinct})---a property of the protocol, designed in rather than an emergent
grace of scale. The deepest exit lies beyond the model class---a
genuinely new \emph{instrument} enlarges the likelihood's own support, and one cannot place a
posterior over the unconceived \citep{stanford2006}. What a community controls is how hard it
looks---and, above all, what its pooling does to the member who finds it first.
...
the dials of Table~\ref{tab:dials}, spanning the baseline and four added mechanisms
(Table~\ref{tab:escapes}).
...
excitation, audit credit, value accretion (Table~\ref{tab:escapes}) \\
\hline
\end{tabular}
...
The sweep's remaining mechanisms sharpen the same conclusion (Table~\ref{tab:escapes}).
None of them lets two connected communities holding the
...
certain---Theorem 2's confident compromise; the gated agent's variance (solid) shows an
...
\input{appendix_crisis_discovery_v5}
