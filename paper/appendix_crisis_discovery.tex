% ======================================================================
% APPENDIX: the crisis and discovery checks, derived from the ledger.
% \input from changing_networked_mind.tex (after the operator appendix).
...
reading.
\item \textbf{The ordering.} Expansion is only attempted strictly after an agent's own crisis.
The narrative justification (the residual the old paradigm absorbed becomes visible only after
reduction) is mechanistically plausible but enforced rather than emergent; letting the check
run pre-crisis and showing the ledger rejects it would upgrade this commitment to a result.
...
\item \textbf{The conviction field in the ledger.} $\Delta U$ is evaluated under the agent's
current conviction field $U=\Top u$ with the intrinsic utility $u$ fixed; agents do not revise
$u$ itself. Motivated agents who re-derive what they value is a different (interesting) model.
\end{itemize}

% ----------------------------------------------------------------------
\subsection{Future work: the rate, made principled}
\label{app:exploration-future}
% ----------------------------------------------------------------------

\paragraph{Exploration as a social process.}
The Poisson proposal rate is deliberately the most agnostic model of conceiving an unconceived
alternative, and it is also where the model is most clearly incomplete: in real communities the
\emph{propensity} to entertain new structure is itself socially modulated. A natural refinement
keeps Stanford's constraint (no agent can plan toward a specific unconceived dimension) while
letting the rate ride the trust graph: a self- and mutually-exciting (Hawkes) process, in which
agent $i$'s rate $r_i(t)$ receives a decaying bump whenever a trusted neighbour expands,
\begin{equation}
r_i(t) \;=\; r_0 \;+\; \sum_{j\in\partial i}\, T_{ij} \sum_{t_j < t} \beta\, e^{-(t-t_j)/\tau},
\end{equation}
with $t_j$ the neighbours' discovery times. This is the structural-proposal channel the model
already has, promoted from carrying \emph{content} (which node to wake) to also carrying
\emph{momentum} (how eagerly to look)---a formal home for the observation that revolutions
arrive in waves. Because the population results show a staggered discovery never survives the
communal prior, an excitation term is not a cosmetic addition: it is the mechanism by which a
community could manufacture the near-simultaneity that survival requires, and the Hawkes
branching ratio would become the model's answer to how much social excitement a revolution
needs.

\paragraph{The rate as a belief: an active-inference closure.}
A second refinement folds the rate back into the formalism. The forgetting factor is already
read as a prior on structural drift (\S\ref{sec:temporal}: one forgets at the rate one believes
the world is moving), and the proposal rate admits the dual reading: one \emph{explores} at the
rate one believes one's model is incomplete. That belief is itself updatable---a hyperprior on
model adequacy, driven by the very residual floor the discovery check records as telemetry---so
that a persistent unexplained residual raises the agent's own arrival rate, while a quiet model
lets it decay. Under that closure the exploration rate stops being a dial and becomes a
posterior; together with the epistemic-gain term already on the ledger, expansion would then be
fully inside the active-inference loop (the agent expands when expected information gain pays
for the inversion, and looks for candidates at a rate set by its inferred inadequacy). We did
not pursue this here because it dissolves the clean waiting-time law into a feedback system,
and because the general problem it gestures at---principled out-of-distribution generalization,
how an agent should behave toward dimensions it cannot yet represent---is well beyond this
paper's scope. The phlogiston world needs only the agnostic rate; the refinement is where this
model meets that open problem.
