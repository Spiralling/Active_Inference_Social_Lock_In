% ======================================================================
% APPENDIX (v2): the crisis and discovery checks, derived from the ledger.
% \input from changing_networked_mind_v2.tex (after the operator appendix).
...
reading.
\item \textbf{The ordering --- now a result, not a commitment.} The first draft enforced that
expansion is attempted only after an agent's own crisis, and flagged that letting the check run
pre-crisis and showing the ledger rejects it would upgrade the commitment to a result. It does:
running the identical proposal + ledger pipeline for every pre-crisis agent at every step
...
\item \textbf{The new mechanisms' own commitments.} The closed-loop refinements of
\S\ref{sec:res-closing} discharge the largest entries of the first draft's list (the constant
rate, the one-shot accept, the static $u$) but import quantitative choices of their own: the
...
The first draft of this appendix sketched two refinements as future work; both are now
mechanisms of the model, exercised in \S\ref{sec:res-closing}. This subsection records the
inferential readings that license them, the audit that the full ledger's acceptance rule makes
necessary, and the accretion dynamics behind the Lakatos result---together with what each leaves
stipulated.
...
decay to the agnostic base. The clean waiting-time law of \S\ref{sec:res-kuhn} does, as
anticipated, dissolve into a feedback system; the experiments therefore report realized
(effective) rates, and the matched-mean control of \S\ref{sec:res-closing} is what separates
...
to how much social excitement a revolution needs (\S\ref{sec:res-closing},
...
The conviction source $u$, exogenous and static in the first draft, gains the slow dynamics the
limitations called for: per node, $u_{\mathrm{eff}} = u\,(1+g)$ with
