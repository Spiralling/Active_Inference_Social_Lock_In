...
onto epoch 0 and never catches the moving truth. Here we add the paper's forgetting factor
omega in (0, 1] (Sec. 3.3), wired into ``simulation.run_simulation``:

    Pi <- Pi_prior + omega (Pi - Pi_prior),   h <- h_prior + omega (h - h_prior),

so a deposit from k steps ago carries weight omega^k. Two consequences, both reported here:

  1. THE WALL BREAKS. With omega < 1 the lever-free population RE-TRACKS each successive theory.
  2. CONVICTION BECOMES MOTIVATED PERSISTENCE. The value tilt h <- h + lambda U re-applies every
     step, so it accumulates even as evidence forgets -- a committed bloc stays on its home theory
     after the evidence that supported it has leaked away. Lock-in becomes a conviction THRESHOLD.

Reuses the cosmology world + population builders of ``src.structural.models.cosmology``.
"""
...
def frac_on_true(r: dict, scn, theory_mu: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """(frac_on_true (S,), true_epoch (S,)): fraction of the population whose mean belief sits
    closest to the CURRENT true theory at each snapshot."""
    means = _agent_means(r)
...
def run_omega(scn, pop, *, omega: float, conviction: float, connected: bool, snap: int,
              seed: int = 0) -> dict:
...
def run(out_dir, params: dict) -> None:
    omegas = tuple(params["OMEGAS"])
    snap = params["SNAP"]
    persist_omega = params["PERSIST_OMEGA"]
    conv_grid = tuple(params["CONV_GRID"])
    ...
    sweep = []
    for om in omegas:
        d = run_omega(scn, pop, omega=om, conviction=0.0, connected=True, snap=snap)
    ...
    # dominates (degenerate lock-in). With forgetting the tilt saturates at lambda*U/(1-omega) and
    # only holds if it beats the (also-saturated) per-step evidence -- so lock-in is a THRESHOLD.
    print(f"\n=== (2) motivated persistence: lock-in is now EARNED (ω={persist_omega:.2f}, disconnected) ===")
    persist = []
    for cv in conv_grid:
        d = run_omega(scn, pop, omega=persist_omega, conviction=cv, connected=False, snap=snap)
    ...
    track12 = [float(np.mean(d["by_epoch"][1:])) for d in sweep]
    ...
    ax.set_xlabel(f"conviction λ  (ω={persist_omega:.2f}, disconnected)")
    ...
    print(f"  no-conviction bloc (ω={persist_omega:.2f}) ends on {EPOCH_NAMES[persist[0]['cons_end']]} "
          f"(expect re-tracks, not dark_matter)")
    print(f"  high-conviction bloc (ω={persist_omega:.2f}, λ={conv_grid[-1]}) ends on "
    ...
    np.savez_compressed(
        ...
        persist_held=np.stack([p["held"] for p in persist]),
...
register(ExperimentSpec(
    model="cosmology",
    name="cosmology_forgetting",
    description="Forgetting (ω<1) breaks the no-forgetting wall (the population re-tracks each "
                "epoch); conviction then becomes motivated persistence with an EARNED lock-in threshold.",
    run=run,
    out_dir="cosmology_forgetting",
    params=dict(OMEGAS=(1.0, 0.97, 0.93, 0.88, 0.80, 0.70), SNAP=5, PERSIST_OMEGA=0.90,
                CONV_GRID=(0.0, 6.0, 15.0, 30.0, 60.0)),
    consumes=dict(figures=["diag_wall_break", "diag_tradeoff", "diag_motivated_persistence"]),
))
