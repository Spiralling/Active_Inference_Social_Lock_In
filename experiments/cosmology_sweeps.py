...
All three reuse the cosmology engine + population builders (no new mechanism) and run WITH
forgetting (omega = 0.9, so the population actually re-tracks the moving world -- otherwise the
no-forgetting wall masks everything), delivered as ONE three-panel figure for the paper.

  E3  CONNECTIVITY (Zollman). Sweep the cross-community bridge density -> algebraic connectivity
      lambda_2. Denser graphs reach consensus faster but shed the transient structural diversity a
      sparser community preserves -- the Zollman speed-vs-diversity trade-off.
  ...
  E5  SHARING CONCLUSIONS vs RAW EVIDENCE. fuse_mode = posterior (share whole belief nets =
      ...
      herds the population (lower diversity).

World constants come from ``src.structural.models.cosmology`` (the single cosmology source); this
module owns the sweep axes, the figure, and the registry spec.
"""
...
def run(out_dir, params: dict) -> None:
    ...
    inters = tuple(params["INTERS"])
    precs = tuple(params["PRECS"])
    modes = tuple(params["FUSE_MODES"])
    ...
    def run_one(spec, graph, *, fuse_mode="posterior", seed=0):
    ...
    for it in inters:
        ...
        for s in seeds:
            ...
            r = run_one(cosmology_spec(scn, pop, conviction=0.0), g, seed=s)
    ...
    for mode in modes:
        fr, div = [], []
        for s in seeds:
            r = run_one(cosmology_spec(scn, pop, conviction=0.0), g_fixed, fuse_mode=mode, seed=s)
...
register(ExperimentSpec(
    model="cosmology",
    name="cosmology_sweeps",
    description="E3-E5: connectivity (Zollman speed-vs-diversity), prior stiffness, and "
                "sharing-conclusions-vs-raw-evidence on the cosmology re-tracking task.",
    run=run,
    out_dir="cosmology_sweeps",
    params=dict(OMEGA=0.9, SEEDS=(0, 1), INTERS=(0.0, 0.01, 0.03, 0.08, 0.2),
                PRECS=(0.5, 1.0, 2.0, 4.0, 8.0),
                FUSE_MODES=("posterior", "deposit_pool", "deposit_keep")),
    seeds=(0, 1),
    consumes=dict(figures=["diag_sweeps"]),
))
