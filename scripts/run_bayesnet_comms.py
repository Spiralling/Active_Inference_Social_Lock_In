...
the symmetric precision Pi and potential h (``step.fuse``, fuse_mode='posterior'). That is the
"Gaussian lambda thing": it shares beliefs, not the directed Bayes net. This script contrasts it
against the new fuse_mode='bayesnet', which communicates the DIRECTED net: each step every agent's
belief is read back as its CPDs (``bayesnet.from_info`` -> parent weights B, intercepts b, residual
variances s), the CPDs are trust-weight-averaged over the graph, and recompiled (``to_info``).
Because the CPD<->precision bijection is nonlinear, averaging directed edge weights B is a genuinely
different operation from averaging Pi.

We run the cosmology re-tracking task (heterogeneous 3-community population, forgetting omega=0.9 so
the population re-tracks the moving world) under BOTH modes, in two regimes -- connected/no-conviction
(pure re-tracking) and disconnected+conviction (the both-lever lock-in) -- and compare re-tracking,
structural diversity, and the held theory. Honest question: does communicating the Bayes net change
the population behavior relative to pooling precision?

Run it::

    python scripts/run_bayesnet_comms.py
"""
...
OMEGA = 0.9
SEEDS = (0, 1, 2)
MODES = ("posterior", "bayesnet")          # precision-lambda vs directed-CPD communication
...
def run_cell(scn, pop, theory_mu, *, connected, conviction, mode, seed):
...
def structural_heterogeneity(scn, pop) -> dict:
    """WHY the two modes coincide: do agents actually hold different directed Bayes nets, or do
    they share a structure and differ only in their means? Run one connected re-tracking cell and,
    at the final step, compare the cross-agent dispersion of the directed edge weights B (the
    structure) against that of the posterior means (the beliefs). If B-dispersion << mean-dispersion,
    there is no structural information to communicate beyond what precision-pooling already shares."""
    ...
    Pi, h = np.asarray(r["snap_Pi"][-1]), np.asarray(r["snap_h"][-1])      # (N,d,d),(N,d)
    B, _, _ = jax.vmap(bn_from_info)(jnp.asarray(Pi), jnp.asarray(h))
    B = np.asarray(B); d = B.shape[-1]
    il = np.tril_indices(d, -1)                                            # directed edges
    B_disp = float(np.mean(np.std(B[:, il[0], il[1]], axis=0)))           # cross-agent std of edges
    means = np.linalg.solve(Pi, h[..., None])[..., 0]                      # (N,d)
    mu_disp = float(np.mean(np.std(means, axis=0)))                        # cross-agent std of beliefs
    return {"B_disp": B_disp, "mu_disp": mu_disp}


def main() -> int:
    out_dir = ROOT / "results" / "bayesnet_comms"
    ...
    for rlabel, kw in regimes:
        print(f"\n=== {rlabel} ===")
        for mode in MODES:
            cells = [run_cell(scn, pop, theory_mu, mode=mode, seed=s, **kw) for s in SEEDS]
    ...
    het = structural_heterogeneity(scn, pop)
    ...
    print(f"  -> agents share the same directed STRUCTURE and differ only in their MEANS, so there is "
          ...
          f"communicating Bayes nets would only matter once agents hold genuinely DIFFERENT learned "
          f"structures (the data-dependent operator of §7.7).")
